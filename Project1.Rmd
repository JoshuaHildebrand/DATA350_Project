...
- Cleaning the data requires selecting the variables Testosterone, Age, Gender, and BMI_WHO. Where Testosterone variable is total testosterone in nanograms per deciliter for participants aged 6 years or older for the years 2011-2012. Age is the age of the participant in 2011-2012, subjects who were 80 years or older were recorded as 80. BMI WHO is the body mass index as categorized by the World Health Organization as 12.0-18.4 (underweight), 18.5-24.9 (normal weight), 25-29.9 (overweight), and 30 plus (Obese). The BMI categories were translated from being denoted as intervals to written as their names to be more accessible to viewers who do not have access to the data, however, the caption contains the details for their reference. Then the data set was filtered for to include on the male gender to restrict the data to just one level of gender. Further analysis should explore testosterone across all genders, however, this focuses on males to avoid confounding influence. The first step of illustrating the answer to the guiding question is to plot the distributions of each BMI group's testosterone over age to understand the concentration and spread of the data.
...
NHANES_t |>
  ...
  labs(title = 'Testosterone vs Age', y = 'Testosterone (ng/dL)',
       col = 'BMI WHO', caption = str_wrap('Source: NHANESraw dataset, BMI WHO Categorized: 12.0-18.4 (underweight), 18.5-24.9 (normal weight), 25-29.9 (overweight), and 30 plus (Obese)', width = 100)) + 
  theme_bw()
```

- The observations' testosterone values are distributed by BMI group over age. The plot succeeds in showing exactly how data points look regarding their testosterone and comparing how each sample contrasts in size and spread. Furthermore, this scatterplot reveals that their distributions are not normal. Thus using the mean would be ill advised and more robust statistics are appropriate for better understanding the distributions. Therefore, to compare each groups testosterone values, median and interquartile range are used to accurately groups testosterone for similar age values.
...
summary_table <- NHANES_t2 |>
  kable(digits = 2, 
        col.names = c("Age", "BMI WHO", "Median Testosterone", "Q1", "Q3"),
        caption = "Summary Statistics for Testosterone by BMI WHO group over Age")
summary_table

```

Age is grouped into approximately twenty year increments starting at the first age recorded for testosterone, which is six years. Analysis of this plot provides insights to answering the guiding question. Comparing across age groups reveals that in general the youngest age group (06-20) has the lowest median testosterone. Comparing across BMI WHO groups reveals that in general the underweight BMI WHO group has the highest median testosterone with the exception of the 06-20 age group. A notable observation is that underweight participants in the 06-20 age group have a median of 3.675 which is abnormally small compared to other groups. Further analysis implies that this subgroup is dominated by the youngest participants in the data which have a very low testosterone compared to older participants in this subgroup which skews the median down. Further analysis beyond comparison requires understanding the influence that these three variable have on each other.

```{r, fig.width=9}
NHANES_cor <- NHANESraw |> 
  mutate(BMI_WHO = as.numeric(BMI_WHO)) |>
  select(Testosterone, Age, BMI_WHO)|>
  na.omit()|>
  cor() 

  
ggcorrplot(NHANES_cor, method = 'square', type = 'upper',
         lab = TRUE, hc.order = TRUE, colors = c( 'red','white',"blue"),
         ggtheme= ggplot2::theme_bw(), legend.title = 'Correlation')+
  labs(title = 'Correlation of Testosterone, Age, and BMI WHO',
       caption = str_wrap('Source: NHANESraw dataset, BMI WHO Numbered: Underweight(1), Normal Weight(2), Overweight(3), Obese(4)', width = 200))

```

- BMI WHO numbered 1-4 for correlation analysis.Underweight(1), Normal Weight(2), Overweight(3), Obese(4). Correlation achieves that last goal of understanding the influence these three variables have on each other. Correlation enhances the exploration of the data by calculating a weak positive correlation across all relationships.BMI WHO and Age have the strongest correlation in the data at 0.41, yet still not enough to claim a strong positive relationship.
